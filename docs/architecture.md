...
- Release/artist roles use a small application-controlled vocabulary, currently distinguishing primary associations from composer appearances.
...
- Container startup baselines databases created before migration history was introduced, then uses `prisma migrate deploy` for versioned production upgrades. Release checks exercise the migration history against a populated database created from the previous release tag.
...
- The production Docker image defaults to one Freshwax service: startup applies the Prisma schema, then runs the web server and BullMQ worker under the same container lifecycle. The release image packages Next.js standalone output, an ahead-of-time worker bundle, and an isolated Prisma CLI so this behavior does not require the full application dependency tree at runtime. This keeps self-hosted Compose installs simple while still allowing non-Docker process managers to run the app and worker separately.
